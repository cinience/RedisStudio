...
{
    if (param.iType == UIEVENT_BUTTONDOWN && param.bLeaf==true)
    {
        TEventUI myEvent;
        myEvent.pSender = param.pSender;
        if (OnClick) OnClick(&myEvent);
    }
	switch (param.wParam)
	{
	case 1:
...
LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	else if( uMsg == WM_KILLFOCUS )
	{
		...
		if( !bInMenuWindowList ) {
			param.wParam = ContextMenuParam::RemoveAll;
            param.iType = 0;
...
void CMenuElementUI::DoEvent(TEventUI& event)
{
	if( event.Type == UIEVENT_MOUSEENTER )
	{
		...
		else
		{
			...
			param.wParam = ContextMenuParam::RemoveSubMenu;
            param.bLeaf = true;
            param.iType = event.Type;
            param.pSender = this;
			s_context_menu_observer.RBroadcast(param);
			m_pOwner->SelectItem(GetIndex(), true);
		}
